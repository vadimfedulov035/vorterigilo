...
// map_set_allocator allows for configuring a custom allocator for
// all map library operations. This function, if needed, should be called
// only once at startup and a prior to calling map_new().
...
// map is an open addressed hash map using robinhood hashing.
struct map {
    void *(*malloc)(size_t);
...
void map_set_load_factor(struct map *map, double factor) {
...
static struct bucket *bucket_at(struct map *map, size_t index) {
...
static uint64_t get_hash(struct map *map, const void *key) {
    return clip_hash(map->hash(key, map->seed0, map->seed1));
}


// map_new_with_allocator returns a new hash map using a custom allocator.
// See map_new for more information information
struct map *map_new_with_allocator(void *(*_malloc)(size_t), 
    ...
    void *udata)
{
    ...
    // map + spare + edata
    size_t size = sizeof(struct map)+bucketsz*2;
    struct map *map = _malloc(size);
    if (!map) {
        return NULL;
    }
    ...
    map->elfree = elfree;
    map->udata = udata;
    map->spare = ((char*)map)+sizeof(struct map);
...
// map_new returns a new hash map. 
// Param `elsize` is the size of each element in the tree. Every element that
// is inserted, deleted, or retrieved will be this size.
// Param `cap` is the default lower capacity of the map. Setting this to
...
// comes with two helper functions `map_sip()` and `map_murmur()`.
// Param `compare` is a function that compares items in the tree. See the 
// qsort stdlib function for an example of how this function works.
// The map must be freed with map_free(). 
...
struct map *map_new(size_t elsize, size_t cap, uint64_t seed0, 
    ...
    void *udata)
{
    return map_new_with_allocator(NULL, NULL, NULL, elsize, cap, seed0, 
        seed1, hash, compare, elfree, udata);
}

static void free_elements(struct map *map) {
    if (map->elfree) {
...
// map_clear quickly clears the map. 
// Every item is called with the element-freeing function given in map_new,
// if present, to free any data referenced in the elements of the map.
...
static bool resize0(struct map *map, size_t new_cap) {
    struct map *map2 = map_new_with_allocator(map->malloc, map->realloc, 
...
static bool resize(struct map *map, size_t new_cap) {
    return resize0(map, new_cap);
}

// map_set_with_hash works like map_set but you provide your
// own hash. The 'hash' callback provided to the map_new function
// will not be called
const void *map_set_with_hash(struct map *map, const void *item,
    uint64_t hash)
{
    hash = clip_hash(hash);
    map->oom = false;
...
// map_set inserts or replaces an item in the hash map. If an item is
// replaced then it is returned otherwise NULL is returned. This operation
// may allocate memory. If the system is unable to allocate additional
// memory then NULL is returned and map_oom() returns true.
const void *map_set(struct map *map, const void *item) {
    return map_set_with_hash(map, item, get_hash(map, item));
}

// map_get_with_hash works like map_get but you provide your
// own hash. The 'hash' callback provided to the map_new function
// will not be called
const void *map_get_with_hash(struct map *map, const void *key, 
    uint64_t hash)
{
    hash = clip_hash(hash);
...
// map_get returns the item based on the provided key. If the item is not
// found then NULL is returned.
const void *map_get(struct map *map, const void *key) {
    return map_get_with_hash(map, key, get_hash(map, key));
}

// map_probe returns the item in the bucket at position or NULL if an item
...
// map_delete_with_hash works like map_delete but you provide your
// own hash. The 'hash' callback provided to the map_new function
// will not be called
const void *map_delete_with_hash(struct map *map, const void *key,
    uint64_t hash)
{
    hash = clip_hash(hash);
    map->oom = false;
...
// map_delete removes an item from the hash map and returns it. If the
// item is not found then NULL is returned.
const void *map_delete(struct map *map, const void *key) {
    return map_delete_with_hash(map, key, get_hash(map, key));
}

// map_count returns the number of items in the hash map.
size_t map_count(struct map *map) {
    return map->count;
}

// map_free frees the hash map
// Every item is called with the element-freeing function given in map_new,
// if present, to free any data referenced in the elements of the map.
...
// map_oom returns true if the last map_set() call failed due to the 
// system being out of memory.
bool map_oom(struct map *map) {
    return map->oom;
}

// map_scan iterates over all items in the hash map
// Param `iter` can return false to stop iteration early.
// Returns false if the iteration has been stopped early.
bool map_scan(struct map *map, 
...
// map_iter iterates one key at a time yielding a reference to an
// entry at each iteration. Useful to write simple loops and avoid writing
// dedicated callbacks and udata structures, as in map_scan.
//
...
// modified.
//
// Note that if map_delete() is called on the map being iterated,
...
// map_sip returns a hash value for `data` using SipHash-2-4.
uint64_t map_sip(const void *data, size_t len, uint64_t seed0,
    uint64_t seed1)
{
    return SIP64((uint8_t*)data, len, seed0, seed1);
}

// map_murmur returns a hash value for `data` using Murmur3_86_128.
uint64_t map_murmur(const void *data, size_t len, uint64_t seed0,
    uint64_t seed1)
{
    (void)seed1;
...
uint64_t map_xxhash3(const void *data, size_t len, uint64_t seed0,
    uint64_t seed1)
{
    (void)seed1;
...
// $ cc -DHASHMAP_TEST map.c && ./a.out              # run tests
// $ cc -DHASHMAP_TEST -O3 map.c && BENCH=1 ./a.out  # run benchmarks
//==============================================================================
#ifdef HASHMAP_TEST

static size_t deepcount(struct map *map) {
    size_t count = 0;
    for (size_t i = 0; i < map->nbuckets; i++) {
...
static uint64_t hash_int(const void *item, uint64_t seed0, uint64_t seed1) {
    return map_xxhash3(item, sizeof(int), seed0, seed1);
    // return map_sip(item, sizeof(int), seed0, seed1);
    // return map_murmur(item, sizeof(int), seed0, seed1);
}

static uint64_t hash_str(const void *item, uint64_t seed0, uint64_t seed1) {
    return map_xxhash3(*(char**)item, strlen(*(char**)item), seed0, seed1);
    // return map_sip(*(char**)item, strlen(*(char**)item), seed0, seed1);
    // return map_murmur(*(char**)item, strlen(*(char**)item), seed0, seed1);
}
...
static void all(void) {
    ...
    assert(map_sip("hello", 5, 1, 2) == 2957200328589801622);
    assert(map_murmur("hello", 5, 1, 2) == 1682575153221130884);
    assert(map_xxhash3("hello", 5, 1, 2) == 2584346877953614258);
    ...
    while (!(map = map_new(sizeof(int), 0, seed, seed, 
                               hash_int, compare_ints_udata, NULL, NULL))) {}
    shuffle(vals, N, sizeof(int));
    for (int i = 0; i < N; i++) {
        // // printf("== %d ==\n", vals[i]);
        assert(map->count == (size_t)i);
        assert(map->count == map_count(map));
        assert(map->count == deepcount(map));
        const int *v;
        assert(!map_get(map, &vals[i]));
        assert(!map_delete(map, &vals[i]));
        while (true) {
            assert(!map_set(map, &vals[i]));
            if (!map_oom(map)) {
                break;
            }
        }
        ...
        for (int j = 0; j < i; j++) {
            v = map_get(map, &vals[j]);
            assert(v && *v == vals[j]);
        }
        while (true) {
            v = map_set(map, &vals[i]);
            if (!v) {
                assert(map_oom(map));
                continue;
            } else {
        ...
        v = map_get(map, &vals[i]);
        assert(v && *v == vals[i]);
        v = map_delete(map, &vals[i]);
        assert(v && *v == vals[i]);
        assert(!map_get(map, &vals[i]));
        assert(!map_delete(map, &vals[i]));
        assert(!map_set(map, &vals[i]));
    ...
    assert(map_scan(map, iter_ints, &vals2));

    // Test map_iter. This does the same as map_scan above.
    size_t iter = 0;
    void *iter_val;
    while (map_iter (map, &iter, &iter_val)) {
    ...
    while (!(map = map_new(sizeof(char*), 0, seed, seed,
...
static void benchmarks(void) {
    ...
    map = map_new(sizeof(int), 0, seed, seed, hash_int, compare_ints_udata, 
                      NULL, NULL);
    bench("set", N, {
        const int *v = map_set(map, &vals[i]);
        assert(!v);
    })
    shuffle(vals, N, sizeof(int));
    bench("get", N, {
        const int *v = map_get(map, &vals[i]);
        assert(v && *v == vals[i]);
    })
    shuffle(vals, N, sizeof(int));
    bench("delete", N, {
        const int *v = map_delete(map, &vals[i]);
        assert(v && *v == vals[i]);
    })
    map_free(map);

    map = map_new(sizeof(int), N, seed, seed, hash_int, compare_ints_udata, 
                      NULL, NULL);
    bench("set (cap)", N, {
        const int *v = map_set(map, &vals[i]);
        assert(!v);
...
int main(void) {
    map_set_allocator(xmalloc, xfree);

    if (getenv("BENCH")) {
        printf("Running map.c benchmarks...\n");
        benchmarks();
    } else {
        printf("Running map.c tests...\n");
        all();
        printf("PASSED\n");
    }
}
